...
void test("sandbox resolves paths inside the selected root", async (context) => {
...
void test("sandbox rejects traversal outside the root", async (context) => {
  const root = await mkdtemp(path.join(tmpdir(), "swiftcoderai-"));
  context.after(async () => rm(root, { recursive: true, force: true }));
  const sandbox = await ProjectSandbox.create(root);
...
void test("sandbox rejects a symlink that escapes the root", async (context) => {
  const root = await mkdtemp(path.join(tmpdir(), "swiftcoderai-"));
  const outside = path.join(tmpdir(), `swiftcoderai-outside-${crypto.randomUUID()}`);
  context.after(async () =>
    Promise.all([rm(root, { recursive: true, force: true }), rm(outside, { force: true })]),
  );
