...
void test("detects exact Swift package commands", async (context) => {
...
void test("distinguishes Xcode projects and rejects marker symlinks", async (context) => {
  const root = await project({});
  const outside = await project({
    "package.json": JSON.stringify({ scripts: { test: "untrusted" } }),
  });
  context.after(async () =>
    Promise.all([
...
void test("uses only declared Node package scripts and identifies TypeScript", async (context) => {
  const root = await project({
...
void test("detects Python, Rust, and Go profiles in mixed projects without enabling arbitrary commands", async (context) => {
  const root = await project({
...
void test("leaves an unrecognized project without executable commands", async (context) => {
...
void test("exposes run_command only with an enum of detected commands", async (context) => {
  ...
  context.after(async () =>
    Promise.all([
      ...
      rm(swiftRoot, { recursive: true, force: true }),
    ]),
  );

  const unknownTools = await ToolExecutor.create(unknownRoot);
  const swiftTools = await ToolExecutor.create(swiftRoot);
  const runCommand = swiftTools.definitions.find(
    (definition) => definition.function.name === "run_command",
  );

  assert.equal(
    unknownTools.definitions.some((definition) => definition.function.name === "run_command"),
    false,
  );
  assert.match(JSON.stringify(runCommand), /"enum":\["swift build","swift test"\]/);
});

void test("runs a detected npm command using the trusted executable path", async (context) => {
  const root = await project({
    "package.json": JSON.stringify({
      scripts: { test: "node -e \"console.log('profile command ran')\"" },
    }),
  });
  context.after(async () => rm(root, { recursive: true, force: true }));
...
void test("hardens Git inspection against configured helper programs", () => {
  assert.deepEqual(hardenedGitArguments(["diff"]), [
    "--no-pager",
    "-c",
