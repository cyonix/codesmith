...
export function formatDebugEvent(event: AgentEvent): string | undefined {
  switch (event.type) {
    ...
    case "tool_started":
      return `start ${event.call.function.name} ${previewSensitiveText(event.call.function.arguments)}`;
    case "tool_finished":
      return `done ${event.call.function.name} ${previewSensitiveText(event.result)}`;
